...
Criterion's own estimate format is distinct from the comparison JSON. The
comparison runner records nearest-rank p50/p95/p99 latency from the declared
sampling interval, total measured operations, elapsed operation time, and
operations/second.
...
scripts/run-benchmark-comparison.sh \
  --engine rocksdb --workload benchmarks/workloads/v1-smoke.json
```

The dependency checker reports every missing tool with an installation hint
and exits before invoking Cargo. The RocksDB build is feature-gated, so normal
workspace builds do not require those native dependencies.
...
- cache layout, Bloom filters, file format, recovery algorithm, and background
  scheduling are engine-specific. `threads` records the foreground runner
  count and maps to RocksDB background jobs, while MeteorDB owns one background
  worker.
